...
def tree_pose(pose):
    right_8 = find_point(pose, 8)
    right_knee = find_point(pose, 9)
    right_10 = find_point(pose, 10)
    left_11 = find_point(pose, 11)
    #angle of right bent leg
    anglerightleg = angle_calc(find_point(pose, 8), find_point(pose, 9), find_point(pose, 10)) #60 to 20
    #angle of left bent leg
    angleleftleg = angle_calc(find_point(pose, 11), find_point(pose,12), find_point(pose, 13)) #60 to 20
    #angle of right hand
    anglerighthand = angle_calc(find_point(pose, 2), find_point(pose, 3), find_point(pose, 4)) #80 to 30
    #angle of left starignt leg
    angleleftlegstraignt = angle_calc(find_point(pose, 11), find_point(pose, 12), find_point(pose, 13)) #180 to 160
    #angle of right staright leg
    anglerightlegstraignt = angle_calc(find_point(pose, 8), find_point(pose, 9), find_point(pose, 10))  # 180 to 160
    #angle of left hand for namaskara
    anglelefthand = angle_calc(find_point(pose, 5), find_point(pose, 6), find_point(pose, 7)) #80 to 30

    head = find_point(pose, 0)
    right_4 = find_point(pose, 4)
    left_7 = find_point(pose, 7)

    # to find the position of hands and head
    head_right = head[1] - right_4[1]  #<-90
    head_left = head[1] - left_7[1]    #<-90
    #finding the midpoint between 8 and 11
    mid = ((left_11[0] + right_8[0]) / 2, (left_11[1] + right_8[1]) / 2)
    #finding the angle between head, nect and midpoint to track standing
    midangle = angle_calc(find_point(pose, 0), find_point(pose, 1), mid)  #160 to 180
    #finding distance between two arms
    both_arms = int(euclidian(find_point(pose, 7), find_point(pose, 4))) #30

    #Condition for tree position 1 shown in blog where hands are below head
    if (((anglerightleg in range(20,60) and angleleftlegstraignt in range(160,180)) or (angleleftleg in range(20,60) and anglerightlegstraignt in range(160,180)))
        and anglerighthand in range(30,80) and anglelefthand in range(30,80) and head_right<-80 and head_left <-80 and both_arms in range(20,40) and midangle in range(160,180)):
        return True
    #condition for tree position 2 where hands are above head
    if (((anglerightleg in range(20,60) and angleleftlegstraignt in range(160,180)) or (angleleftleg in range(20,60) and anglerightlegstraignt in range(160,180)))
        and anglerighthand in range(120,180) and anglelefthand in range(120,180) and head_right>80 and head_left >80 and both_arms in range(20,60) and midangle in range(160,180)):
        return True
